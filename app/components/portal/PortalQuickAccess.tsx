      ...
      {items.map((item) => {
        ...
        return (
          <Link
            key={item.id}
            to={item.href}
            className="group block min-h-[44px] rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2E75BD]/40 focus-visible:ring-offset-2"
          >
            <PortalCard className="h-full transition-transform duration-200 group-hover:-translate-y-0.5 group-focus-visible:-translate-y-0.5">
              <div className="flex items-start justify-between gap-3">
                <span className="flex size-11 items-center justify-center rounded-xl bg-[#eff6ff] text-[#2E75BD] transition-colors duration-200 group-hover:bg-[#2E75BD] group-hover:text-white">
                  <Icon className="size-5" aria-hidden="true" />
                </span>
                <ArrowUpRight className="size-4 shrink-0 text-gray-300 transition-colors duration-200 group-hover:text-[#2E75BD]" />
              </div>
